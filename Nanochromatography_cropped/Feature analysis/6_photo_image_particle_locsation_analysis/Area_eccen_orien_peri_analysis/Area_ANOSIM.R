...
dir.create("Area_ANOSIM")

list_ = list('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I')


sink('Area_ANOSIM/MDS_anosim_total.txt')

for (val in list_){
  filename = paste('Area_MDS_coordinates/', val, '.csv', sep = '')
  ...
  filename_figure = paste('Area_ANOSIM/Area_MDS_plot_', val, '.tiff')
...
sink('Area_ANOSIM/CMDS_anosim_total.txt')
...
for (val in list_){
  ...
  # "mountford", "raup" , "binomial", "chao", "cao" or "mahalanobis".
  # plot(dispersion, hull=FALSE, ellipse=TRUE)
  filename_figure = paste('Area_ANOSIM/Area_CMDS_plot_', val, '.tiff')
  tiff(filename_figure, units="in", width=10, height=7, res=600)
  plot(res)
  dev.off()
}
