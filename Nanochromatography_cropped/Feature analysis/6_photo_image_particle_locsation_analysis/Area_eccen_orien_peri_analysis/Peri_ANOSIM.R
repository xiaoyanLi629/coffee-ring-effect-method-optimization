...
dir.create("Peri_ANOSIM")

list_ = list('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I')


sink('Peri_ANOSIM/MDS_anosim_total.txt')

for (val in list_){
  filename = paste('Perimeter_MDS_coordinates/', val, '.csv', sep = '')
  ...
  filename_figure = paste('Peri_ANOSIM/Peri_MDS_plot_', val, '.tiff')
...
sink('Peri_ANOSIM/CMDS_anosim_total.txt')
...
for (val in list_){
  ...
  # "mountford", "raup" , "binomial", "chao", "cao" or "mahalanobis".
  # plot(dispersion, hull=FALSE, ellipse=TRUE)
  filename_figure = paste('Peri_ANOSIM/Peri_CMDS_plot_', val, '.tiff')
  tiff(filename_figure, units="in", width=10, height=7, res=600)
  plot(res)
  dev.off()
}
